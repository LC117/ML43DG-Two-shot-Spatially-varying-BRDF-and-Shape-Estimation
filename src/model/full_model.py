...
def full_inference(path_to_default_img, path_to_flash, path_to_mask):
    ...
    location = str(pathlib.Path(path_to_default_img).parent)
    assert location == str(pathlib.Path(path_to_flash).parent)
    ...
    save(normal, str(location / r"normal_pred0.exr"))
    save(depth, str(location / r"depth_pred0.exr"))
